...
export function initialSoundCluster(word) {
...
export function assertAlliterativeName(name) {
    ...
    if (!adjSound || adjSound !== nounSound) {
        throw new Error(`"${name}" is not alliterative ("${adjective}" ${adjSound} vs "${nounWord}" ${nounSound}).`);
    }
}
export function isAlliterativeName(name) {
    try {
