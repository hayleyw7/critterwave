...
export function getPlayerHypeGain(response) {
...
export function formatDanceHypeTail(playerGain, foeJoins) {
    if (playerGain === 0) {
        return "";
    }
    if (foeJoins) {
...
export function resetDancePicker() {
    lastDanceResponseIndex = -1;
    lastDanceOpenerIndex = -1;
}
function pickIndexAvoidingRepeat(length, lastIndex, random) {
    if (length <= 1) {
        return 0;
    }
...
export function pickRandomDanceResponse(random = Math.random) {
    const index = pickIndexAvoidingRepeat(DANCE_RESPONSES.length, lastDanceResponseIndex, random);
    lastDanceResponseIndex = index;
    return DANCE_RESPONSES[index];
}
export function pickRandomDanceOpener(random = Math.random) {
    const index = pickIndexAvoidingRepeat(DANCE_OPENERS.length, lastDanceOpenerIndex, random);
